                            ...
                            for(auto cl : contlist){
                                // cerr<<"loop3";
                                    ...
                                    {                                    
                                        ...
                                        pair<int, int> hp2cl=coordinatebackConversion(cl.second.first, cl.second.second);
                                        pair<int, int> hptbr=coordinatebackConversion(tbr.first, tbr.second);
                                        string move =  "RS "+to_string(hp1cl.first)+" "+to_string(hp1cl.second)+" RE "+to_string(hp2cl.first)+" "+to_string(hp2cl.second)+" X "+to_string(hptbr.first)+" "+to_string(hptbr.second);     
                                        hey = hey + " " +move;
                                        auto temptempboard=tempboard->copyBoard();
                                        ChangeBoard(ismyturn, temptempboard, move);
                                        auto tempcontlist = CheckContMarker(ismyturn, temptempboard);
                                        vector<pair<int, int> > temptempringsabell;
                                        int ringout;
                                        if(ismyturn)   { 
                                            temptempringsabell = temptempboard->myringpos;
                                            ringout = temptempboard->myringout;
                                        }
                                        else {
                                            temptempringsabell = temptempboard->oppringpos;
                                            ringout = temptempboard->oppringout;
                                            }
                                        if(tempcontlist.size()==0 || ringout>=3){
                                            moves.push_back(hey);    
                                        }
                                        else{
                                            for(auto tcl : tempcontlist){
                                                for(auto ttbr : temptempringsabell){
                                                    pair<int, int> thp1cl=coordinatebackConversion(tcl.first.first, tcl.first.second);
                                                    pair<int, int> thp2cl=coordinatebackConversion(tcl.second.first, tcl.second.second);
                                                    pair<int, int> thptbr=coordinatebackConversion(ttbr.first, ttbr.second);
                                                    string move =  " RS "+to_string(thp1cl.first)+" "+to_string(thp1cl.second)+" RE "+to_string(thp2cl.first)+" "+to_string(thp2cl.second)+" X "+to_string(thptbr.first)+" "+to_string(thptbr.second);     
                                                    hey = hey + move;      
                                                    moves.push_back(hey);    
                                                }
                                            }
